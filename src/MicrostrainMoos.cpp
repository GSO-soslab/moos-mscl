...
bool MicrostrainMoos::OnStartUp() {
    ...
    if(!m_MissionReader.GetConfigurationParam("filter_prefix", m_raw_prefix)) {
        m_raw_prefix = "MS";
    }
    ...
    m_name_f_z_quat = m_filter_prefix + "_W_QUAT";
    m_name_f_x_quat = m_filter_prefix + "_X_QUAT";
    m_name_f_y_quat = m_filter_prefix + "_Y_QUAT";
    m_name_f_z_quat = m_filter_prefix + "_Z_QUAT";

    m_name_f_x_quat = m_filter_prefix + "_X_MAG";
    m_name_f_y_quat = m_filter_prefix + "_Y_MAG";
    m_name_f_z_quat = m_filter_prefix + "_Z_MAG";
...
void MicrostrainMoos::publish_filtered(imu_data_t d) {
    if(m_publish_filter) {
        ...
        Notify(m_name_f_x_accel, d.linear_accel_x);
        Notify(m_name_f_y_accel, d.linear_accel_y);
        Notify(m_name_f_z_accel, d.linear_accel_z);

        Notify(m_name_f_x_gyro, d.angular_vel_x);
        Notify(m_name_f_y_gyro, d.angular_vel_y);
        Notify(m_name_f_z_gyro, d.angular_vel_z);

        Notify(m_name_f_x_mag, d.mag_x);
        Notify(m_name_f_y_mag, d.mag_y);
        Notify(m_name_f_z_mag, d.mag_z);

        Notify(m_name_f_w_quat, d.quat_w);
        Notify(m_name_f_x_quat, d.quat_x);
        Notify(m_name_f_y_quat, d.quat_y);
        Notify(m_name_f_z_quat, d.quat_z);
    }
}
